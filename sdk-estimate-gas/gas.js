...
const argv = yargs
  .option('network', {
    // All of those choices are Patex:
    // sepolia - Patex Sepolia, the main test network
    choices: ["sepolia"],
    description: 'Patex network to use'
  }).
  option('verify', {
    type: boolean,
...
const getSigner = async () => {
  ...
  if (argv.network == 'sepolia') {
    endpointUrl = process.env.PATEX_SEPOLIA_URL
  }
  const l2RpcProvider = patexSDK.asL2Provider(
